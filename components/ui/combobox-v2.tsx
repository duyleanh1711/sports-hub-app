...
import { ScrollArea } from '@/components/ui/scroll-area-v2';
    ...
    <div className="relative not-has-[>*.w-full]:w-fit w-full has-disabled:opacity-64">
      {startAddon && (
        <div
          aria-hidden="true"
          className={cn(
            'pointer-events-none absolute inset-y-0 start-px z-10 flex items-center gap-2',
            'ps-[calc(--spacing(3)-1px)] opacity-80',
      ...
      <ComboboxPrimitive.Input
        className={cn(
          startAddon &&
            'data-[size=sm]:*:data-[slot=combobox-input]:ps-[calc(--spacing(7.5)-1px)] *:data-[slot=combobox-input]:ps-[calc(--spacing(10.5)-1px)] sm:data-[size=sm]:*:data-[slot=combobox-input]:ps-[calc(--spacing(7)-1px)] sm:*:data-[slot=combobox-input]:ps-[calc(--spacing(9)-1px)]',
          sizeValue === 'sm'
...
function ComboboxEmpty({ className, ...props }: ComboboxPrimitive.Empty.Props) {
  return (
    <ComboboxPrimitive.Empty
      className={cn(
        'not-empty:p-12 text-center text-base text-muted-foreground sm:text-sm',
        className
      )}
